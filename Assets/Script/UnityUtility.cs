...
//・ダウンロードしてきたアセットバンドルファイルから、Luaファイルを読み込む
	...
	public static bool IsUseLocalStreamingAssets = true;
	...
	public static int UnityLoadPrefabAfter(IntPtr luaState)
	{
		...
		if (IsUseLocalStreamingAssets == true) {
			retObj = GameObjectCacheManager.Instance.LoadGameObject(assetBundleName+"/"+path, objectName);
		} else {
...
//#endif
	...
	public static int UnityDebugLog(IntPtr luaState)
	{
		...
		Debug.Log(logstring + ":timestamp " + Time.realtimeSinceStartup);
...
//		loadPath = "jar:file://" + filePath;
		loadPath = "jar:file://" + Application.dataPath + "!/assets" + "/LuaMain.lua";
		//loadPath = "file:///" + filePath;
		//loadPath = filePath;
#elif UNITY_IPHONE
		//loadPath = "jar:file://" + filePath;
